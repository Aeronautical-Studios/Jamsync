...
async function setup() {
  ...
  if (window.location.pathname.includes("committedfile")) {
    ...
    const currentCommitResp = await fetch(`/api/commits/${owner}/${projectName}`);
    ...
    fileResp = await fetch(
      `/api/workspacefile/${owner}/${projectName}/${workspaceId}?path=${currentPath}&changeId=${currentChangeId}`,
    );
  }
